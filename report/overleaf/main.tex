...
This project evaluated if simulated vehicle movement can be improved by learning unmodeled attributes from vehicle specific data. Results show that training Long Short Term Memory (LSTM) Recurrent Neural Networks (RNN) for end-to-end learning of vehicle movement can output next movements with plausible results for some movement features, but that end-to-end learning results alone do not appear to outperform physics modeling. LSTM experiments were performed with the input of variable lengths of recent history from vehicle movement and vehicle control input, and output as the next update for vehicle movement. Related work indicates that joint usage of physics modeling and novel LSTM architectures can achieve better results than physics modeling alone.

\section{Related work}

\subsection{Vehicle dynamics modeling}

The field of vehicle dynamics modeling has been well studied \cite{guiggiani2014science, jazar2017vehicle, popp2010ground, schramm2014vehicle} and is yielding excellent results. However these techniques are limited by the complexity that is manually added into the model, can be time consuming and expensive to tune for a real-world vehicle, and for real-time applications accuracy is competing with the available real-time compute budget.

\subsection{End-to-end deep learning vehicle dynamics}

\cite{bojarski2016end} explores end-to-end Deep Learning (DL) all the way from camera pixels to vehicle actuation output, this does not focus specifically on vehicle dynamics but presents a surprisingly successful extreme for the limit of end-to-end DL in vehicles.

\subsection{Unifying physics and deep learning for physical movement}

In \cite{shi2018neural, zeng2019tossingbot} techniques are presented to unify physics models with DNN's, with results outperforming physics models alone. \cite{shi2018neural} presents a nominal dynamics model combined with a DNN to learn how to control multi-rotor drone movement and landing. This solution utilizes modeling for the well known aspects of the problem however shows that extending modeling with a DNN resulted in better performance than a baseline control system, and performance generalized better to data outside the training domain.
...
A visual analysis was performed to attempt to identify a reasonable actuation response delay, this method was modified to add columns to the input data containing the difference between the current time step and multiple past time steps (e.g. (current - 10ms ago), (current - 20ms ago), etc.). This modification was quickly abandoned in favor of pursuing a network architecture that could learn temporal sequences.
...
\begin{figure}[h!]
  \centering
  \begin{minipage}[b]{0.49\textwidth}
    \includegraphics[width=\textwidth]{LSTM_arch}
...
Appendix \ref{appendix:models} shows a table of attempted model parameters, hyperparameters, and input/output data sets (further defined in Appendix \ref{appendix:data}), and the resulting loss and mean absolute error for each model.
...
\end{enumerate}
 ...
 \begin{figure}[h!]
  \centering
  \begin{minipage}[b]{0.49\textwidth}
    \includegraphics[width=\textwidth]{model80/out_yawAngle.png}
    \caption{Model 80 yaw angle test set actual vs predicted.}
    \label{fig:model80_yaw}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.49\textwidth}
    \includegraphics[width=\textwidth]{model80/out_vxCG.png}
    \caption{Model 80 longitudinal velocity test set actual vs predicted.}
    \label{fig:model80_vx}
  \end{minipage}
\end{figure}
...
\begin{figure}[h!]
  \centering
  \begin{minipage}[b]{0.49\textwidth}
    \includegraphics[width=\textwidth]{model80/out_vyCG.png}
    \caption{Model 80 lateral velocity test set actual vs predicted.}
    \label{fig:model80_vy}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.49\textwidth}
    \includegraphics[width=\textwidth]{model80/out_wheelAccelFL.png}
    \caption{Model 80 front left wheel acceleration test set actual vs predicted.}
    \label{fig:model80_wheel}
  \end{minipage}
\end{figure}
 
 Figures \ref{fig:model80_yaw}, \ref{fig:model80_vx}, \ref{fig:model80_vy}, \ref{fig:model80_wheel} are zoomed-out plots of Model 80 incrementally predicting the next value of a test set file. Figures \ref{fig:model80_zoom_vx}, \ref{fig:model80_zoom_yaw}, \ref{fig:model80_zoom_wheel} are zoomed-in plots of Model 80 predictions. These figures visualize that different vehicle dynamic features were learned with variable success: 
 \begin{itemize}
     \item \textbf{longitudinal velocity}: (Figures \ref{fig:model80_vx} and \ref{fig:model80_zoom_vx}) Mean Squared Error (MSE) of 1.88 with an input value range of 61.72, resulting in MSE of 3.04\% of the input range. Longitudinal velocity appears to have been successfully learned as hoped due to strong correlation with the throttle and brake pedal input, after some time delay. However the model appears to accumulate most of its error when input values go beyond 50, future work would be needed to understand why the model appears to be failing to output values much beyond 50.
     \item \textbf{lateral velocity}: (Figure \ref{fig:model80_vy}) MSE of 0.27 on input range of 4.04, resulting in MSE of 6.91\% of range. Lateral velocity was nowhere near as successful at being learned as longitudinal velocity, this is likely due to a weaker correlation between steering input and lateral velocity as well as a significantly smaller input range for this feature.
     \item \textbf{yaw angle}: (Figures \ref{fig:model80_yaw} and \ref{fig:model80_zoom_yaw}) MSE of 2.96 on input range of 359.98, resulting in MSE of 0.82\% of range. Yaw angle appears to have been quite successfully learned as hoped due to strong correlation with the steering wheel angle. It is surprising how well the model was able to handle yaw angle discontinuities when it exceeds +/-180, how this is being accomplished would benefit from further investigation.
     \item \textbf{wheel acceleration}: (Figures \ref{fig:model80_wheel}, \ref{fig:model80_zoom_wheel}) MSE of 1.53 on input range of 83.88, resulting in MSE of 1.83\% of range. Wheel acceleration data appears to be reporting with too high of a frequency for the model, which is resulting in the model appearing to low-pass filter this feature.
 \end{itemize}

\begin{figure}[!ht]
\includegraphics[width=\textwidth, height=5cm]{model80/zoom_vxCG.png}
\caption{Model 80 zoomed-in longitudinal velocity test set actual vs predicted.}
\label{fig:model80_zoom_vx}
\end{figure}

\begin{figure}[!ht]
\includegraphics[width=\textwidth, height=5cm]{model80/zoom_yawAngle.png}
\caption{Model 80 zoomed-in yaw angle test set actual vs predicted.}
\label{fig:model80_zoom_yaw}
\end{figure}

\begin{figure}[!ht]
\includegraphics[width=\textwidth, height=5cm]{model80/zoom_wheelAccelFL.png}
\caption{Model 80 zoomed-in front left wheel acceleration test set actual vs predicted.}
\label{fig:model80_zoom_wheel}
\end{figure}

\section{Conclusion/Future Work }
From these results LSTM's outperformed attempts to hand engineer temporal relationships into the data then train with simple multi-layer networks, and LSTM models appear generally capable of learning vehicle dynamics with temporal delays. However, LSTM results still had a pretty significant MSE between predicted future results and actual future results in the test set, so the investigated network architecture does not appear to be a candidate to threaten replacing hand modeling vehicle dynamics for simulation.

For future work I would like to investigate:
\begin{enumerate}
    \item \textbf{Integrating physical modeling with a LSTM}: as proposed by \cite{shi2018neural} and \cite{zeng2019tossingbot}. The approach proposed in both \cite{shi2018neural} and \cite{zeng2019tossingbot} is to utilize a physics model to implement the known features of movement then integrating with a learning model to fine-tune the unknown or hardware unit specific features. Specifically, I would like to integrate a TensorFlow LSTM model with the PyBullet \cite{bullet} physics simulator's vehicle model, then apply common steering / brake / throttle input in simulation as from a real-world data capture and plot the resulting differences in position and orientation over time.
    \item \textbf{Predict movement farther into the future}: this project only predicted the next step of movement based on recent history, it would be interesting to understand how much future movement could be accurately predicted based on the recent past.
\end{enumerate}
